...
<div class="title-container">
    <h1 style="text-align:center">Tabel Achizitii</h1>
</div>
...
<?php if ($result->num_rows > 0): ?>
  <?php while ($row = $result->fetch_assoc()): ?>
    <tr>
      ...
      <?php
      $getProviderNameQuery = "SELECT nume FROM furnizori WHERE id_furnizor = '" . $row['id_furnizor'] . "';";
      $getProviderNameResult = mysqli_query($conn,$getProviderNameQuery);
      $providerName = mysqli_fetch_assoc($getProviderNameResult);
      ?>
      <td><?php echo $providerName["nume"]; ?></td>
      <td><?php echo $row["email_angajat"]; ?></td>
      <td><?php echo $row["valoare"]; ?></td>
      <td><?php echo $row["data"]; ?></td>
      <td style="text-align:center">
      <a href="show_order_provider.php?id_achizitie=<?php echo $row['id_achizitie'];?>">
          <button class="btn btn-info">Detalii Achizitie</button></a>
      </td>
    </tr>
  <?php endwhile; ?>
<?php else: ?>

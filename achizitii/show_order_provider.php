...
<div class="title-container">
    <h1 style="text-align:center">Detalii achizitie</h1>
</div>
...
<?php if (isset($_GET['id_achizitie'])): ?>
   <?php $id_achizitie = $_GET['id_achizitie'];

$query="SELECT * FROM produse_achizitii WHERE id_achizitie = '$id_achizitie';";
$result = mysqli_query($conn,$query);?>
<?php if ($result->num_rows > 0): ?>
  <?php while ($row = $result->fetch_assoc()): ?>
    <?php
    $productID = $row["id_produs"];
    $queryProductName = "SELECT nume FROM produse WHERE id_produs = '$productID';";
    $resultProductName = mysqli_query($conn,$queryProductName);
    if($resultProductName->num_rows > 0){
        $productName = mysqli_fetch_assoc($resultProductName)['nume'];
    }else{
        $productName = 0;
    }

    $categoryID = $row["id_categorie"];
        $queryCategoryName = "SELECT nume FROM categorii WHERE id_categorie = '$categoryID';";
        $resultCategoryName = mysqli_query($conn,$queryCategoryName);
        if($resultCategoryName->num_rows > 0){
            $categoryName = mysqli_fetch_assoc($resultCategoryName)['nume'];
        }else{
            $categoryName = 0;
        }
    ?> 
    <tr>
      <td><?php echo $row["id_achizitie"]; ?></td>
...
<?php else: ?>
  <tr>
    <td style="text-align:center" colspan="6">Achizitia nu exista</td>
  </tr>
<?php endif; ?>
<?php endif; ?>
</tbody>
